...
with st.expander("اضغط هنا لفتح أداة الالتقاط اليدوية", expanded=False):
    ...
    if capture_btn and tweet_url:
        if APIFLASH_KEY == "ضع_مفتاحك_هنا":
            ...
            with st.spinner("جاري التقاط الصورة بدقة عالية وحجم كبير... ⏳"):
                try:
                    encoded_url = urllib.parse.quote(tweet_url)
                    
                    # الرابط الجديد: دمج 800x600 مع scale_factor=2
                    api_url = f"https://api.apiflash.com/v1/urltoimage?access_key={APIFLASH_KEY}&url={encoded_url}&width=800&height=600&format=png&delay=3&scale_factor=2&response_type=image"
    ...
    if st.session_state.snapshot_img:
        ...
        # تم إرجاع العرض ليتناسب مع الشاشة بدلاً من تقييده بـ 550
        ...
        with col_down:
            st.download_button(
                ...
                file_name=f"tweet_snap_{int(time.time())}.png",
                mime="image/png",
