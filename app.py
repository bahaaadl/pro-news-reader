...
st.subheader("📸 استخراج لقطة شاشة نقية (نص كامل وتصميم مثالي)")
...
with st.expander("اضغط هنا لفتح أداة الالتقاط اليدوية", expanded=False):
    ...
    if capture_btn and tweet_url:
        if APIFLASH_KEY == "ضع_مفتاحك_هنا":
            ...
            # استخراج الـ ID من الرابط العادي
            match = re.search(r"status/(\d+)", tweet_url)
            if match:
                tweet_id = match.group(1)
                with st.spinner("جاري التقاط التغريدة الأصلية بنقاء عالي... ⏳"):
                    try:
                        # السر هنا: نعود لاستخدام رابط التضمين النقي الذي يعرض النص الطويل تلقائياً
                        target_url = f"https://platform.twitter.com/embed/Tweet.html?id={tweet_id}&lang=ar&theme=light"
                        encoded_url = urllib.parse.quote(target_url)
                        
                        # رابط ApiFlash نظيف بدون أكواد JS معقدة تكسر التصميم
                        api_url = f"https://api.apiflash.com/v1/urltoimage?access_key={APIFLASH_KEY}&url={encoded_url}&format=png&delay=3&width=550&scale_factor=2&response_type=image"
                        
                        resp = requests.get(api_url)
                        if resp.status_code == 200:
                            st.session_state.snapshot_img = resp.content
                        else:
                            st.error(f"❌ فشل الالتقاط. تأكد من أن مفتاحك صحيح وأن التغريدة موجودة.")
                    except Exception as e:
                        st.error(f"حدث خطأ: {e}")
            else:
                st.warning("⚠️ الرابط غير صحيح. يرجى التأكد من نسخ رابط التغريدة كاملاً من تويتر.")
                    
    if st.session_state.snapshot_img:
        st.success("✅ تم الالتقاط بنجاح (تصميم مثالي ونص كامل)!")
        st.image(st.session_state.snapshot_img, width=550)
        ...
        with col_down:
            st.download_button(
                ...
                file_name=f"tweet_perfect_{int(time.time())}.png",
                mime="image/png",
