...
# ==========================================
# --- أداة التقاط التغريدات (النسخة الاحترافية - توافق تام مع ApiFlash) ---
# ==========================================
import urllib.parse
import time
import requests
        ...
        if APIFLASH_KEY == "ضع_مفتاحك_هنا":
            ...
            with st.spinner("جاري فتح التغريدة وتطبيق إعدادات ApiFlash القياسية... ⏳"):
                try:
                    ...
                    # 1. إخفاء بانر التسجيل والنوافذ السفلية باستخدام خاصية hide_elements الرسمية
                    elements_to_hide = "#layers, [data-testid='bottom-banner']"
                    encoded_hide = urllib.parse.quote(elements_to_hide)
                    
                    # 2. كود JS بسيط فقط للضغط على زر "عرض المزيد"
                    js_click = """
                    setTimeout(() => {
                    ...
                    # 4. الرابط السحري: 
                    # - width=1024 (تصميم كمبيوتر عريض لمنع ضغط النص)
                    # - scroll_page=true (لضمان تحميل كل شيء)
                    # - delay=5 (لإعطاء وقت كافٍ لفتح النص الطويل)
