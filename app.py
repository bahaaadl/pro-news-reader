...
with st.expander("اضغط هنا لفتح أداة الالتقاط اليدوية", expanded=False):
    ...
    if capture_btn and tweet_url:
        if APIFLASH_KEY == "ضع_مفتاحك_هنا":
            ...
            # 1. استخراج رقم التغريدة (ID) من الرابط العادي
            ...
            if match:
                ...
                with st.spinner("جاري التقاط التغريدة الأصلية بنقاء عالي... ⏳"):
                    try:
                        # 2. استخدام فكرتك العبقرية (رابط التضمين النقي)
                        # يمكنك تغيير lang=ar إلى lang=en إذا أردت الواجهة بالإنجليزية
                        target_url = f"https://platform.twitter.com/embed/Tweet.html?id={tweet_id}&lang=ar"
                        encoded_url = urllib.parse.quote(target_url)
                        
                        # الرابط مع إعداداتك المفضلة (width=550 & scale_factor=2)
                ...
                st.warning("⚠️ الرابط غير صحيح. يرجى التأكد من أنه يحتوي على رقم التغريدة (status/ID).")
                    
    # عرض الصورة المحفوظة مع زر تحميل
    if st.session_state.snapshot_img:
        st.success("✅ تم الالتقاط بنجاح وبدون أي بنرات!")
        st.image(st.session_state.snapshot_img, width=550) # استخدمنا 550 ليطابق حجمك المفضل
        ...
        with col_down:
            st.download_button(
                ...
                file_name=f"tweet_{int(time.time())}.png",
                mime="image/png",
